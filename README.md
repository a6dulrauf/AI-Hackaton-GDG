...
```bash
# 1. Generate the synthetic donor dataset (run once)
cd data
python generate_donors.py            # writes donors.csv (220 rows)
...
> The `dev.ps1` launcher handles this for you.
...
├── data/
│   ├── generate_donors.py   # synthetic dataset generator (pure stdlib)
│   └── donors.csv           # generated — gitignored output
├── backend/                 # FastAPI app (Python)
│   ├── main.py              # app, CORS, startup CSV load, endpoints
...
- **No database** — donors load from `data/donors.csv` into memory at startup.
...
> `/health` and `/donors` are live now; the rest are built in order per [`docs/PLAN.md`](docs/PLAN.md) §8.

## Troubleshooting

- **"Python was not found"** — the Windows Store stub is on PATH but real Python isn't.
  Install it (`winget install Python.Python.3.12`) and re-run `.\dev.ps1`, which finds it automatically.
